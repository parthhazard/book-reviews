...
def create_app():
    ...
    class registrationForm(Form):
        ...
        genre_list = ['Action and Adventure', 'Alternate History', 'Art', 'Autobiography', 'Biography', 'Cookbook', 'Crime', 'Diary', 'Drama', 'Fantasy', 'Graphic Novel', 'Health', 'History', 'Horror', 'Mystery',
            'Poetry', 'Religion, Spirutiality and new age', 'Romance', 'Satire', 'Science Fiction', 'Self Help', 'Short Story', 'Suspense', 'Thriller', 'Travel', 'True Crime']
    ...
    def dashboard():
        ...
        db.commit()
        reviewQuery = db.execute("SELECT reviews.*, books.title FROM reviews INNER JOIN books ON reviews.isbn = books.isbn WHERE reviews.user_id = :id", {"id": user_id}).fetchall()
    ...
    @app.route('/updatereview/<string:isbn>/<int:id>', methods=['POST'])
    @isLoggedIn
    def updateReview(isbn, id):
        if request.method == 'POST':
